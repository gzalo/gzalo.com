<?php
	$descripcionPagina = '';
	$tituloPagina = 'Timer overflow calculator';
	$lang = 'en';	
	
	echo addBoxBeg('Timer overflow calculator');
?>
...
function compute(){
	...
	$("#ciclo").val(calc(1/c) + "seconds");
	
	var prescaler = $("#prescaler").val();
	
	$("#tickTimer").val(calc(prescaler/c) + "seconds");
	
	var reload = $("#reload").val();
	
	$("#rH").val(parseInt(reload / 256) + " " + reload % 256);
	
	$("#timeover").val(calc((65535-reload)*prescaler/c) + "seconds");
